	    ...
	    return {
			get: function(successCallback) {
				...
				.success(function(data) {
					successCallback(data.LineStatus, new Date(), 'Online');
					localStorageService.set('ngTFLStatus', data.LineStatus);
					localStorageService.set('ngTFLTimeStamp', new Date());
				}).
				error(function() {
					console.log('ERROR: Could not get data.');
					successCallback(localStorageService.get('ngTFLStatus'),  Date.parse(localStorageService.get('ngTFLTimeStamp')), 'Offline');
				});
			}
		};
	});
